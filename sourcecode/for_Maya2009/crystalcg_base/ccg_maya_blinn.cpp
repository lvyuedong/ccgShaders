...
extern "C" DLLEXPORT void ccg_maya_blinn_init(      /* init shader */
    miState         *state,
    struct ccg_maya_blinn *paras,
    miBoolean       *inst_req)
{
	if (!paras) *inst_req = miTRUE;
			...
			*fbarray = (struct ccg_passfbArray *)mi_mem_allocate(sizeof(struct ccg_passfbArray));
			...
			{
				if(!fbc) mi_error("Please set the fbWriteString parameter for ccg_maya_blinn shader.");
				else {
...
extern "C" DLLEXPORT void ccg_maya_blinn_exit(      /* exit shader */
	miState         *state,
	struct ccg_maya_blinn *paras)
{
	if(paras)
...
extern "C" DLLEXPORT int ccg_maya_blinn_version(void) {return(1);}

extern "C" DLLEXPORT miBoolean ccg_maya_blinn(
  miColor   *result,
  miState   *state,
  struct ccg_maya_blinn *paras)
{
	...
	miScalar	eccen, rolloff, refracti, fkr, reflcoeff;
	...
	miVector	E, N;	/* E:eye ray direction; N:normalized normal vector; */
	miScalar	EN;		/* dot product of E and N */
	miColor		tempColor;
	...
	if(!passesOnce)
	{
		...
		if(fresnelMode>0)
		{
			...
			switch(fresnelMode){
				...
				case 2:	//apply standard fresnel coefficient
						...
						break;
				//case 3: //look up table of standard fresnel coefficient
				//		if(nIncidence<nTransmission && nTransmission>=1.1 && nTransmission<2.6)
				//		{
				//			row = ceil(nTransmission*10.0 - 11);
				//			col = floor(ccg_clamp(acos(-state->dot_nd>0.999999?0.999999:-state->dot_nd)/CCG_PI*180.0,0,90));
				//			fkr = (*fresnelLUT)[row][col];
				//		}else fkr = ccg_fresnel_reflectance(state, nIncidence, nTransmission);
				//		break;
			}
		...
		eccen	=	*mi_eval_scalar(&paras->eccentricity);
		rolloff = *mi_eval_scalar(&paras->specularRolloff);
		...
		E = state->dir;
		mi_vector_neg(&E);
		mi_vector_normalize(&E);
		N = state->normal;
		mi_vector_normalize(&N);
		EN = mi_vector_dot(&E, &N);
		
		if (fabs(eccen) == 1.0f) eccen = 0.999999f;
		eccen = 1.0f / (SQR(eccen) - 1.0f);
		
		if((spec.r==0.0&&spec.g==0.0&&spec.b==0.0)||eccen==0.0) specEntry = 0;
		else specEntry = 1;
				...
				for (mi::shader::LightIterator iter(state, light, n_l);!iter.at_end(); ++iter){
					...
					while (iter->sample()) {
						...
						if(emitSpecular && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry) {
								s = ccg_SC_specular_blinn(eccen, rolloff, N, dir, E, dot_nl, EN);
								if (s > 1e-5) {
				...
				if(mi_trace_environment(&lightColor, state, &bent_i)) {
					...
					if(dot_nl>0)
					{
						...
						if(ibl_emit_spec && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry) {
								s = ccg_SC_specular_blinn(eccen, rolloff, N, bent_i, E, dot_nl, EN);
								if (s > 1e-5) {
		...
		reflcoeff = ccg_SC_reflect_blinn(rolloff, EN);
		reflecti.r *= spec.r * reflcoeff;
		reflecti.g *= spec.g * reflcoeff;
		reflecti.b *= spec.b * reflcoeff;
		...
		if(whichlayer==LAYER_refl || whichlayer==LAYER_combined)
		{
			if(!ccg_color_compare(&reflecti, &ccg_solid_black))
			{
				if(!rayRefl && (!state->inv_normal || state->reflection_level<in_refl))
				{
					...
					//we look at state->child to decide whether reflect ray hit object
		...
		if(whichlayer == LAYER_ao || (amb_add && whichlayer == LAYER_combined))
		{
			if(bent_normal.a!=-1) ccg_color_init(&passes[LAYER_ao], bent_normal.a);
					...
					state_copy = *orig_state;
					state = &state_copy;
					passes[LAYER_ao] = *mi_eval_color(&paras->ambientOcclusion);
					state = (miState*)orig_state;
				}
		...
		if(whichlayer == LAYER_reflao || (amb_add && whichlayer == LAYER_combined))
		{
			...
			state_copy = *orig_state;
			state = &state_copy;
			passes[LAYER_reflao] = *mi_eval_color(&paras->reflectOcclusion);
			state = (miState*)orig_state;
