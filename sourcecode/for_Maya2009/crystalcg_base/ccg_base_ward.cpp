...
extern "C" DLLEXPORT void ccg_base_ward_init(      /* init shader */
    miState         *state,
    struct ccg_base_ward *paras,
    miBoolean       *inst_req)
{
	if (!paras) *inst_req = miTRUE;
			...
			*fbarray = (struct ccg_passfbArray *)mi_mem_allocate(sizeof(struct ccg_passfbArray));
			...
			{
				if(!fbc) mi_error("Please set the fbWriteString parameter for ccg_base_ward shader.");
				else {
...
extern "C" DLLEXPORT void ccg_base_ward_exit(      /* exit shader */
	miState         *state,
	struct ccg_base_ward *paras)
{
	if(paras)
...
miScalar ccg_ward_specular(miState *state, miVector X, miVector Y, miVector *L, miScalar *dot_nl, miScalar *xroughness, miScalar *yroughness)
{
	miScalar	cos_theta_r, rho, XH, YH, HN;
	miVector	H,V;
	
	V = state->dir;
	mi_vector_neg(&V);
	cos_theta_r = ccg_clamp(-state->dot_nd,0.0001f,1);
	mi_vector_div(&X, *xroughness);
	mi_vector_div(&Y, *yroughness);
	if((*dot_nl)>0.0)
	{
		mi_vector_add(&H, &V, L);
		mi_vector_normalize(&H);
		XH = mi_vector_dot(&X, &H);
		YH = mi_vector_dot(&Y, &H);
		HN = mi_vector_dot(&H, &state->normal);
		rho = exp(-2*(pow(XH,2)+pow(YH,2))/(1+HN))/sqrt((*dot_nl)*cos_theta_r);
	}
	return (rho*(*dot_nl)/(4*(*xroughness)*(*yroughness)));
}


extern "C" DLLEXPORT int ccg_base_ward_version(void) {return(1);}

extern "C" DLLEXPORT miBoolean ccg_base_ward(
  miColor   *result,
  miState   *state,
  struct ccg_base_ward *paras)
{
	...
	if(!passesOnce)
	{
		...
		if(whichlayer==LAYER_z) {
			...
			z = (miScalar(state->dist) - zmin)/(zmax - zmin);
		...
		/* calculate tangent U and V in current intersection point */
		const miVector	*t[3],*p[3];
		miVector	u_dir, v_dir;
		miBoolean	exUV = *mi_eval_boolean(&paras->externalUV);
		if(!exUV)
		{
			if(mi_tri_vectors(state, 't', 0, &t[0], &t[1], &t[2]))
			{
				if(ccg_illegalTex(*t[0],*t[1],*t[2])&&mi_tri_vectors(state, 'p', 0, &p[0], &p[1], &p[2]))
				{
					ccg_getTangentUV(state, 0, *t[0], *t[1], *t[2], *p[0], *p[1], *p[2], &u_dir, &v_dir);
					miScalar xangle = *mi_eval_scalar(&paras->angle);
					...
					u_dir = *mi_eval_vector(&paras->U);
					v_dir = *mi_eval_vector(&paras->V);
				}

		miScalar xrough = *mi_eval_scalar(&paras->xroughness);
		miScalar yrough = *mi_eval_scalar(&paras->yroughness);
			...
			if (m == 4 || n_l){
				for (mi::shader::LightIterator iter(state, light, n_l);!iter.at_end(); ++iter){
					...
					while (iter->sample()) {
						...
						/* specular reflection: ward specular model */
						if(emitSpecular && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry){
								s = ccg_ward_specular(state, u_dir, v_dir, &dir, &dot_nl, &xrough, &yrough);
								if (s > 0.0) {
				...
				if(mi_trace_environment(&lightColor, state, &bent_i)) {
					...
					if(dot_nl>0)
					{
						...
						if(ibl_emit_spec && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry) {
								s = ccg_ward_specular(state, u_dir, v_dir, &bent_i, &dot_nl, &xrough, &yrough);
								if (s > 0.0f) {
