/**************************************************
Source code originally comes from ccg_texture_mix20layers.cpp by Jan Sandstrom (Pixero)
...
struct ccg_texture_mix20layers {
  //global attributes
...
extern "C" DLLEXPORT int ccg_texture_mix20layers_version() { return 1;}

void ccg_texture_mix20layers_rgb2hsv(miColor i, miColor *o)
{
  float min, max, delta;
...
void ccg_texture_mix20layers_hsv2rgb(miColor i, miColor *o)
{
  int h;
  float f,p,q,t,sf;
...
void ccg_texture_mix20layers_colorCorrection(miColor *i, miScalar h, miScalar s, miScalar v, miScalar gamma)
{
  miColor hsv;
...
void ccg_texture_mix20layers_contrastFormula(miScalar *i, miScalar contrast, miScalar power)
{
  float value;
...
void ccg_texture_mix20layers_contrastBrightness(miColor *i, miScalar contrast, miScalar brightness)
{
  if(brightness!=1){
    i->r *= brightness;
    i->g *= brightness;
  ...
  i->b = i->b>1?1:(i->b<0?0:i->b);
    ...
    ccg_texture_mix20layers_contrastFormula(&(i->r), contrast, power);
    ccg_texture_mix20layers_contrastFormula(&(i->g), contrast, power);
    ccg_texture_mix20layers_contrastFormula(&(i->b), contrast, power);
  }
}

void ccg_texture_mix20layers_colorInvert(miColor *i)
{
  i->r = 1 - i->r;
  i->g = 1 - i->g;
  i->b = 1 - i->b;
...
extern "C" DLLEXPORT miBoolean ccg_texture_mix20layers(
  miColor   *result,
  miState   *state,
  struct    ccg_texture_mix20layers *paras)
{
  miColor LayerResult;
  ...
  for (int i = 0; i < n_layers; i++)
  {
    ...
    ccg_texture_mix20layers_colorInvert(&opac); //invert transparency for maya
    ...
    if(*mi_eval_boolean(&paras->alphaInvert[i])) {
      ccg_texture_mix20layers_colorInvert(&opac);
      opac.a = 1 - opac.a;
    }
    ...
    //color correct?
      if(*mi_eval_boolean(&paras->enable_cc[i])){
        ccg_texture_mix20layers_colorCorrection(&B, *mi_eval_scalar(&paras->hue[i]),
      ...
      //inverse color?
      if(*mi_eval_boolean(&paras->colorInvert[i])) ccg_texture_mix20layers_colorInvert(&B);
  ...
  if(*mi_eval_boolean(&paras->g_enable_cc)){
    ccg_texture_mix20layers_colorCorrection(&LayerResult, *mi_eval_scalar(&paras->g_hue),
                          *mi_eval_scalar(&paras->g_saturation),
                          *mi_eval_scalar(&paras->g_value),
                          *mi_eval_scalar(&paras->g_gamma) );
    ccg_texture_mix20layers_contrastBrightness(&LayerResult, *mi_eval_scalar(&paras->g_contrast),
