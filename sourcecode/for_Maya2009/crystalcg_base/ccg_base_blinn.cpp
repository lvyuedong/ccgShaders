...
extern "C" DLLEXPORT void ccg_base_blinn_init(      /* init shader */
    miState         *state,
    struct ccg_base_blinn *paras,
    miBoolean       *inst_req)
{
...
extern "C" DLLEXPORT void ccg_base_blinn_exit(      /* exit shader */
	miState         *state,
	struct ccg_base_blinn *paras)
{
	if(paras)
...
miScalar ccg_specular_mayablinn (
	miScalar *ecc,
	miScalar *roll,
	miVector *N,
	miVector *L,
	...
	miScalar *EN ) 
{
	...
	HN = mi_vector_dot(N, &H);
	EH = mi_vector_dot(E, &H);

	b = SQR((*ecc + 1.0f) / (*ecc + SQR(HN)));
...
extern "C" DLLEXPORT int ccg_base_blinn_version(void) {return(1);}

extern "C" DLLEXPORT miBoolean ccg_base_blinn(
  miColor   *result,
  miState   *state,
  struct ccg_base_blinn *paras)
{
	...
	miScalar	eccen, rolloff, refracti, fkr;
				...
				for (mi::shader::LightIterator iter(state, light, n_l);!iter.at_end(); ++iter){
					...
					while (iter->sample()) {
						...
						if(emitSpecular && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry) {
								s = ccg_specular_mayablinn(&eccen, &rolloff, &N, &dir, &E, &dot_nl, &EN);
								if (s > 0.0) {
					...
					if(dot_nl>0)
					{
						...
						if(ibl_emit_spec && (whichlayer==LAYER_combined || whichlayer==LAYER_spec)) {
							if(specEntry) {
								...
								mi_vector_normalize(&N);
								EN = mi_vector_dot(&E, &N);
								s = ccg_specular_mayablinn(&eccen, &rolloff, &N, &bent_i, &E, &dot_nl, &EN);
								if (s > 0.0f) {
