...
//disable warning convertion from size_t to int
#pragma warning(disable : 4267)

struct ccg_base_template_x {
	miInteger	blendMode;
	miColor		color;
	...
	miColor		diffuse;
	miColor		diffuse_without_shadow;
...
extern "C" DLLEXPORT void ccg_base_template_x_init(      /* init shader */
    miState         *state,
    struct ccg_base_template_x *paras,
    miBoolean       *inst_req)
{
	if (!paras) *inst_req = miTRUE;
			...
			*fbarray = (struct ccg_passfbArray *)mi_mem_allocate(sizeof(struct ccg_passfbArray));
			...
			{
				if(!fbc) mi_error("Please set the fbWriteString parameter for ccg_base_template_x shader.");
				else {
...
extern "C" DLLEXPORT void ccg_base_template_x_exit(      /* exit shader */
	miState         *state,
	struct ccg_base_template_x *paras)
{
	if(paras)
...
extern "C" DLLEXPORT int ccg_base_template_x_version(void) {return(1);}

extern "C" DLLEXPORT miBoolean ccg_base_template_x(
  miColor   *result,
  miState   *state,
  struct ccg_base_template_x *paras)
{
	...
	if((state->type != miRAY_EYE && state->type != miRAY_TRANSPARENT && state->type != miRAY_REFRACT) || state->reflection_level>0){
		...
		//state->user_size = LAYER_combined;
	}
	...
	if(!passesOnce)
	{
		...
		if(whichlayer==LAYER_nor) {
			...
			tempColor.a = 1;
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&tempColor, &copyright);
		...
		{
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_ambi], &copyright);
		...
		if(whichlayer==LAYER_col)
		{
			...
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_col], &copyright);
		...
		if(whichlayer==LAYER_incan)
		{
			...
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_incan], &copyright);
		...
		{
			passes[LAYER_sssfront] = *ccg_mi_eval_color(state, &paras->sss_front);
		...
		{
			passes[LAYER_sssmiddle] = *ccg_mi_eval_color(state, &paras->sss_middle);
			...
			if(whichlayer==LAYER_sssmiddle)
			{
				if(itransp.a!=1.0){
					if(enableTransPass){
						...
						passes[LAYER_sssmiddle].r = itransp.r * passes[LAYER_sssmiddle].r + transp->r * tempColor.r;
						passes[LAYER_sssmiddle].g = itransp.g * passes[LAYER_sssmiddle].g + transp->g * tempColor.g;
						passes[LAYER_sssmiddle].b = itransp.b * passes[LAYER_sssmiddle].b + transp->b * tempColor.b;
					}else {
		...
		{
			passes[LAYER_sssback] = *ccg_mi_eval_color(state, &paras->sss_back);
		...
		if(whichlayer==LAYER_combined || whichlayer==LAYER_diff || whichlayer==LAYER_spec){
			...
			{
				passes[LAYER_diff] = *ccg_mi_eval_color(state, &paras->diffuse);
			}
			if (whichlayer==LAYER_combined || whichlayer==LAYER_spec)
			{
				passes[LAYER_spec] = *ccg_mi_eval_color(state, &paras->specular);
			}
		}
		...
		if(whichlayer==LAYER_diff)
		{
			...
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_diff], &copyright);
		...
		if(whichlayer==LAYER_spec)
		{
			...
			//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_spec], &copyright);
		...
		if(whichlayer==LAYER_refl || whichlayer==LAYER_combined)
		{
			...
			{
				passes[LAYER_refl] = *ccg_mi_eval_color(state, &paras->reflection);
			...
			if(whichlayer==LAYER_refl)
			{
				...
				//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_refl], &copyright);
		...
		{
			if(useRefraShader){
				passes[LAYER_refr] = *ccg_mi_eval_color(state, &paras->refraction);
		...
		if(whichlayer==LAYER_translucent || whichlayer==LAYER_combined){
			...
			passes[LAYER_translucent] = *ccg_mi_eval_color(state, &paras->translucency);
		...
		if(whichlayer==LAYER_globillum || whichlayer==LAYER_combined){
			...
			if(useGlobalShader){
				...
				passes[LAYER_globillum] = *ccg_mi_eval_color(state, &paras->globalIllum);
		...
		passes[LAYER_combined].a = itransp.a + passes[LAYER_refr].a;
				...
				if(mystate->raystate==LAYER_combined || mystate->raystate==LAYER_diff || mystate->raystate==LAYER_spec || mystate->raystate==LAYER_RAY || mystate->raystate==LAYER_withoutShadow)
				{
					...
					if(mystate->raystate==LAYER_RAY || mystate->raystate==LAYER_withoutShadow)
					{
							...
							passes[LAYER_diff] = *ccg_mi_eval_color(state, &paras->diffuse_without_shadow);
					...
					if(mystate->raystate==LAYER_combined || mystate->raystate==LAYER_RAY || mystate->raystate==LAYER_diff || mystate->raystate==LAYER_spec)
					{
							...
							{
								passesCombined[LAYER_diff] = *ccg_mi_eval_color(state, &paras->diffuse);
							...
							{
								passesCombined[LAYER_spec] = *ccg_mi_eval_color(state, &paras->specular);
				...
				if(state->type==miRAY_EYE)
				{
					//ccg_color_divide(&withShadow, &passes[LAYER_diff], &passes[LAYER_shad]);
					...
					{
						//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_shad], &copyright);
				...
				{
					miColor *reflecti =  mi_eval_color(&paras->reflectivity);
					if (!ccg_color_compare(reflecti, &ccg_solid_black))
					{
						int tmpInt = mystate->raystate;
						mystate->raystate = LAYER_combined;
						passesCombined[LAYER_refl] = *ccg_mi_eval_color(state, &paras->reflection);
						if(state->type==miRAY_EYE)	mystate->raystate = LAYER_RAY;
				...
				{
					if(useRefraShader){
						passesCombined[LAYER_refr] = *ccg_mi_eval_color(state, &paras->refraction);
				...
				{
					tempColor = *mi_eval_color(&paras->translucencyMultiply);
					passesCombined[LAYER_translucent] = *ccg_mi_eval_color(state, &paras->translucency);
				...
				if(mystate->raystate==LAYER_combined || mystate->raystate==LAYER_RAY){
					...
					tempColor = *mi_eval_color(&paras->globalIllumMultiply);
					if(useGlobalShader){
						passesCombined[LAYER_globillum] = *ccg_mi_eval_color(state, &paras->globalIllum);
				...
				if(state->type == miRAY_EYE)
				{
					...
					{
						//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_ao], &copyright);
					...
					{
						//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_reflao], &copyright);
				...
				{
					//copyright: if(!ccg_color_compare(&copyright, &ccg_trans_black)) ccg_color_assign(&passes[LAYER_combined], &copyright);
